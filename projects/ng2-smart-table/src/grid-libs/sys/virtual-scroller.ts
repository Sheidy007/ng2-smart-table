import { ChangeDetectorRef, Component, ContentChild, DoCheck, ElementRef, EventEmitter, HostBinding, Inject, Input, NgModule, NgZone, OnChanges, OnDestroy, OnInit, Optional, Output, PLATFORM_ID, Renderer2, ViewChild } from '@angular/core';
...
@Component({
  ...
  template: `
		...
		<div class="ngx-rt-scrollable-content" #content>
  ...
  styles: [`
    ...
    .ngx-rt-scrollable-content ::ng-deep > * {
    ...
    :host.horizontal .ngx-rt-scrollable-content {
      display: flex;
    }

    :host.horizontal .ngx-rt-scrollable-content ::ng-deep > * {
      flex-shrink: 0;
      flex-grow: 0;
      white-space: initial;
    }

    :host.horizontal.rtl .ngx-rt-scrollable-content ::ng-deep > * {
      transform: scaleX(-1);
    }
...
export class VirtualScrollerComponent implements OnInit, OnChanges, OnDestroy, DoCheck {
  ...
  @HostBinding('class.selfScroll')
  get class_selfScroll() {
  ...
  @ViewChild('contentMargin', { read: ElementRef, static: true })
  protected contentMarginRef: ElementRef;
  ...
  public scrollInto(item: any, alignToBeginning: boolean = true, additionalOffset: number = 0): void {
    const index: number = this.items.indexOf(item);
    if (index === -1) {
      return;
    }

    this.scrollToIndex(index, alignToBeginning, additionalOffset);
  }

  public scrollToIndex(index: number, alignToBeginning: boolean = true, additionalOffset: number = 0): void {
    let maxRetries = 5;
    ...
    const retryIfNeeded = () => {
      ...
      this.scrollToIndex_internal(index, alignToBeginning, additionalOffset);
    };

    this.scrollToIndex_internal(index, alignToBeginning, additionalOffset);
  }

  protected scrollToIndex_internal(index: number, alignToBeginning: boolean = true, additionalOffset: number = 0): void {

    const dimensions = this.calculateDimensions();
    let scroll = this.calculateMargin(index, dimensions) + additionalOffset;
    if (!alignToBeginning) {
  ...
  protected debounce(func: () => void, wait: number): () => void {
  ...
  protected throttleTrailing(func: () => void, wait: number): () => void {
    let timeout;
  ...
  protected refresh_internal(itemsArrayModified: boolean): void {
    ...
    this.zone.runOutsideAngular(() => {
      requestAnimationFrame(() => {
        ...
        const paddingChanged = viewport.margin !== this.previousViewPort.margin;
        ...
        if (scrollLengthChanged) {
          ...
          this.scrollLength = viewport.scrollLength;
        }

        if (paddingChanged) {
          if (this.useMarginInsteadOfTranslate) {
            this.renderer.setStyle(this.contentMarginRef.nativeElement, this._marginDir, `${ viewport.margin }px`);
          } else {
            this.renderer.setStyle(this.contentElementRef.nativeElement, 'transform', `${ this._translateDir }(${ viewport.margin }px)`);
            this.renderer.setStyle(this.contentElementRef.nativeElement, 'webkitTransform', `${ this._translateDir }(${ viewport.margin }px)`);
          }
        }
        ...
        if (this.headerElementRef) {
          ...
          const offset = Math.max(scrollPosition - viewport.margin - containerOffset + this.headerElementRef.nativeElement.clientHeight, 0);
  ...
  protected countItemsPerWrapGroup(): number {
    ...
    const children = ( content as HTMLElement ).getElementsByClassName(this.childrenClass);
    ...
    while (result < childrenLength) {
      const offsetTop = children[result][propertyName];
      if (firstOffset === offsetTop) {
        ++result;
      } else {
        break;
      }
  ...
  protected calculateDimensions(): IDimensions {
    ...
    if (this.isAngularUniversalSSR) {
      ...
      if (( content as HTMLElement ).getElementsByClassName(this.childrenClass).length > 0) {
        ...
        const child = ( content as HTMLElement ).getElementsByClassName(this.childrenClass)[0] as HTMLElement;
      ...
      let scrollOffset = scrollElement[this._scrollType] - ( this.previousViewPort ? this.previousViewPort.margin : 0 );
      ...
      for (const child of Array.from(( content as HTMLElement ).getElementsByClassName(this.childrenClass))) {
        ++arrayStartIndex;
        const clientRect = this.getElementSize(child as HTMLElement);
  ...
  protected calculateMargin(arrayStartIndexWithBuffer: number, dimensions: IDimensions): number {
    if (dimensions.itemCount === 0) {
      return 0;
    }

    const defaultScrollLengthPerWrapGroup = dimensions[this._childScrollDim];
    const startingWrapGroupIndex = Math.round(arrayStartIndexWithBuffer / dimensions.itemsPerWrapGroup) || 0;
  ...
  protected calculatePageInfo(scrollPosition: number, dimensions: IDimensions): IPageInfo {
    ...
    let arrayStartIndex = Math.min(Math.round(startingArrayIndex_fractional), maxStart);
    ...
    let arrayEndIndex = arrayStartIndex + dimensions.itemsPerPage - 1;
  ...
  protected calculateViewport(): IViewport {
    ...
    const newMargin = this.calculateMargin(pageInfo.startIndexWithBuffer, dimensions);
