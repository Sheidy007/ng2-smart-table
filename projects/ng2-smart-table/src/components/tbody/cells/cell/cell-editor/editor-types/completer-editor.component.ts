...
@Component({
  ...
  template: `
		<ng2-completer (click)="editClick.emit($event)"
		               [(ngModel)]="this.cell.cellValue.editedValue "
...
export class CompleterEditorComponent extends DefaultEditorComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.cell.getColumn().editor && this.cell.getColumn().editor.type === 'completer') {
      this.config = this.cell.getColumn().columnFunctions.getEditorConfig().completer;
